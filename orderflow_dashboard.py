# orderflow_dashboard_enhanced.py
import json
import time
import pandas as pd
import numpy as np
...
# 保存订单簿变化历史
depth_changes = collections.deque(maxlen=1000)  # 新增：订单簿变化历史
...
def calculate_metrics():
    if not depth_data or not trade_data:
        return
    ...
    # 当前最新价格
    latest_price = float(recent_trades[-1]['p']) if recent_trades else 0
...
tabs_children = [
    ...
    dcc.Tab(label='基本分析', children=[
        html.Div([
            ...
            # 新增的OFI(Order Flow Imbalance)图表
            html.Div([
...
app.layout = html.Div([
    html.H1("比特币订单流分析仪表盘", className='dashboard-title'),
...
def update_charts(n):
    ...
    fig1 = make_subplots(rows=2, cols=1,
                         ...
                         subplot_titles=('BTC/USDT价格 (带入场出场信号)', 'Delta(买卖差值)'),
                         row_heights=[0.7, 0.3])
    ...
    return fig1, fig2, fig3, fig4, signals_table, trade_decisions_display, trade_history_table
